---
title: Quatrième
hide:
  - toc
---

# Espace Quatrième

Choisis une matière, puis un chapitre. Tu arriveras sur le récapitulatif avec les accès à la fiche, au quiz et au cours.

[Révision du jour](revision-du-jour.md){ .md-button .md-button--primary }

## Français

<div class="chapter-list" markdown>

- [***Happy Meal* · Anna Gavalda**](francais/happy-meal-anna-gavalda-recap.md) — Point de vue, fausses pistes et chute.
- [**La nouvelle réaliste et *La Parure***](francais/nouvelle-realiste-la-parure-recap.md) — Maupassant et le portrait de Mathilde Loisel.

</div>

## Anglais

<div class="chapter-list" markdown>

- [**Le présent simple**](anglais/present-simple-recap.md) — Habitudes, questions et fréquence.
- [**Dates and describing a picture**](anglais/dates-description-image-recap.md) — Dates, positions et actions en cours.

</div>

## Mathématiques

<div class="chapter-list" markdown>

- [**Addition et soustraction de relatifs**](mathematiques/addition-soustraction-relatifs-recap.md) — Signes, opposés et simplification.
- [**Théorème de Pythagore**](mathematiques/theoreme-pythagore-recap.md) — Hypoténuse et calcul de longueurs.
- [**Repérage et constructions géométriques**](mathematiques/reperage-espace-constructions-recap.md) — Solides, coordonnées et triangles.

</div>

## SVT

<div class="chapter-list" markdown>

- [**Plaques lithosphériques et séismes**](svt/plaques-lithospheriques-seismes-recap.md) — Activité interne, ondes sismiques et prévention des risques.

</div>